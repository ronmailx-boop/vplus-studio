...
- הדר האתר משתמש ב-`assets/vplus-studio-wordmark-dark.svg` (טקסט לבן) ישירות על הרקע הכהה - קריא בלי צורך ב-badge.
- **תוקן באג גלילה אופקית (horizontal scroll) במובייל צר** (בדקתי בפועל 280-360px): הגורם היה כפתור המייל בסקשן Contact (`.email-link`) - כתובת המייל היא מחרוזת ארוכה בלי רווחים שלא יכלה להיגלל לשורה חדשה, מה שאילץ רוחב מינימלי גדול מרוחב המסך. תוקן עם `overflow-wrap: anywhere` ו-`max-width: 100%`. כמו כן נוסף breakpoint ב-400px שמערים את ההדר (לוגו מעל, ניווט מתחת) במסכים צרים מאוד.
